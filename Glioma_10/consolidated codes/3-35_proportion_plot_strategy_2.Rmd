---
title: "alternative proportion plots"
...
```{r}
indir <- "data/"
outdir <- "proportion_plots_V2/"
if (!dir.exists(outdir)){
  dir.create(outdir)
} 
```

# create newer version of subsets (only whole_tissue) 

For the alternative proportion analysis, CD45_only batch is excluded because the proportion calculation is unfair to the whole tissues. Tumor_7 is also removed

```{r}
whole_meta <- read.table(file = paste0(indir,"whole_meta_annotation_V1.tsv"),sep = "\t",header = T) %>% filter(annotation_V1 != "tumor_7" & batch_ID != "batch_CD45")
...
outdir <- paste0("proportion_plots_V2/",compartment,"/")
