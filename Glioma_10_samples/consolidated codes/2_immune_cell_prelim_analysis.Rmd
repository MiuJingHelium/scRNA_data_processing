---
title: "2_Immune_cell_processing"
output: html_document
date: "2024-05-24"
---

This markdown file contains codes downstream of the the subset creation that generated V3 immune cell subsets. This is a later consolidated version of the immune cell processing, because the original V3 processing and annotation step were performed inside the preliminary functional analysis Rmd file. The codes are directly copy and pasted from the preliminary functional analysis (archived version).

Note that immune cell subset V2 is loaded as the starting point for this step. Immune cell V2 was created from re-processing the the V1 immune cell subset. In fact, there was a confusion in version labeling that all subsets after doublet-removal should be named version 2 to distinguish them from the version from which they were just isolated from the whole dataset. The idea is that these subsets contain different cells such that they are no longer able to recover the V1 whole dataset and hence should no longer be named as V1. However, I was unaware of this issue and named all subsets as V1. The Immune cell V2 is created to remediate this confusion although it is the same dataset as "V1" if the actual immune cell V1 was not recovered.
