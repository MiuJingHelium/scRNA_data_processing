...
## cross condition DE for total mono-mac
...
```
```{r}
mono_mac <- subset(mono_mac, subset = annotation_V2 != "Proliferating Mono/Mac")
...
de_clusters <- function(current_cluster) {
  ...
  both_markers <- FindMarkers(mono_mac, ident.1=groups[2], ident.2=groups[1], test.use="MAST", min.pct=0.00, logfc.threshold = 0) # change name of object 
...
```{r}
mono_mac$celltype <- gsub("/","-",mono_mac$celltype)
mono_mac$group <- gsub(" ","_",paste0(mono_mac$orig.ident,"_",mono_mac$celltype))
conditions <- unique(mono_mac$orig.ident)
clusters <- "Mono-Mac"
plts <- list()
padj_thres = 10e-2
log2FC_thres = 1
Idents(mono_mac) <- mono_mac$group
```
...
```{r}
mm_cond_DE <- read.table(paste0("cross_cond_DE/",paste0('ZBTB46_vs_Control_', "Mono-Mac", '.tsv')),sep = "\t",header = T) %>% filter(p_val_adj < 0.05)
```


```{r}
mm_hallmarks <- fgsea_res(as.character(unique(mm_cond_DE$cluster)),mm_cond_DE,mode = "simple")
```

```{r}
mm_cond_KEGG <- fgsea_res(as.character(unique(mm_cond_DE$cluster)),mm_cond_DE,mode = "simple",gene_set = fgsea_sets_KEGG)
```

```{r}
mm_cond_reactome <- fgsea_res(as.character(unique(mm_cond_DE$cluster)),mm_cond_DE,mode = "simple",gene_set = fgsea_sets_REACT)
```

No need for table consolidation because there is only one cluster.
...
```{r}
EC_cond_DE <- read.table("expression_table_and_markers/ZBTB46_vs_Control_stromal_2.tsv",sep = "\t",header = T)
```
