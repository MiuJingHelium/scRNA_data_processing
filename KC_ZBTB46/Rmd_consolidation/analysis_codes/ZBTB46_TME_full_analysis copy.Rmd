...
### The following is for another project
but the code should provide some ideas of adjusting the visualization
```{r}
plotData <- (jsonlite::fromJSON(file.path("/home/carisa/wustl/monocle","plot_data.json"))$data) %>% 
  as.data.frame
```

```{r}
table(plotData$Sample)
```

```{r}
monocle_palette <- brewer.pal(n = 8, name = "Set1")[c(1,4,2)]
```

```{r}
plotData$condition <- plotData$Sample %>% case_match(
  "KO_ctrl_Female" ~ "KO CTRL",
  "KO_cysf_Female" ~ "KO CysF",
  "WT_ctrl_Female" ~ "WT CTRL",
  "WT_cysf_Female" ~ "WT CysF"
)
```


```{r}
g <- ggplot(plotData,aes(x = data_dim_1,y = data_dim_2,color = as.factor(seurat_clusters)))+
  geom_point()+
  facet_grid(~condition)+theme_classic()+scale_color_manual(values = monocle_palette)+ guides(color=guide_legend(title="Cluster"))
g
ggsave("monocle_updated_plot.pdf",plot = g, units = "cm",height = 20,width = 60)
```
```{r}
g <- plotData %>% filter(condition == "KO CTRL" | condition == "KO CysF") %>% ggplot(aes(x = data_dim_1,y = data_dim_2,color = as.factor(seurat_clusters)))+
  geom_point(size = 0.8)+
  facet_grid(~condition)+ theme_classic()+theme(strip.background = element_blank(),strip.placement = "outside",axis.title = element_blank())+scale_color_manual(values = monocle_palette)+ guides(color=guide_legend(title="Cluster",override.aes=list(fill=NA)))+xlab("Monocle dim 1") +ylab("Monocle dim 2")
g
ggsave("monocle_updated_plot_KO.pdf",plot = g, units = "cm",height = 10,width = 20)
```
